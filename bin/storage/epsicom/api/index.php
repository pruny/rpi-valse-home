...
/*
ArrestDB::Serve('GET', '/(#any)/(#num)/(#any)/(#num)', function ($table, $id, $field1, $value1)
{
	//return "test";
	$query = array
	(
		//sprintf('UPDATE "%s" SET %s WHERE "%s" = ? and "%s" = "%s"', $table, implode(', ', $data), 'id', $field1, $value1),
		sprintf('SELECT * FROM "%s" WHERE "%s" = "%s" and "%s" = "%s"', $table, 'id', $id, $field1, $value1),
		//sprintf('SELECT * FROM "%s" WHERE "%s" = ? and "%s" = ?', $table, 'id', $field1),
		//sprintf('SELECT * FROM "%s"', $table),
		//sprintf('WHERE "%s" %s %s and %s = %s', $id, (ctype_digit($data) === true) ? '=' : 'LIKE', $id, $field1, $value1),
	);

//return sprintf('SELECT * FROM "%s" WHERE "%s" = "%s" and "%s" = "%s"', $table, 'id', $id, $field1, $value1);
...
//ArrestDB::Serve('GET', '/(#any)/(#any)/(#any)/(#num)', function ($table, $id, $field1, $value1)
ArrestDB::Serve('GET', '/(#any)/(#num)/(#any)/(#any)', function ($table, $id, $field1, $value1)
{
	//return "TEST";
	$query = array
	(
		sprintf('SELECT * FROM "%s"', $table),
		sprintf('WHERE "%s" %s ? and id="%s"', $field1, (ctype_digit($value1) === true) ? '=' : 'LIKE', $id),
	);
	...
	$query = sprintf('%s;', implode(' ', $query));
	$result = ArrestDB::Query($query, $value1);
...
ArrestDB::Serve('DELETE', '/(#any)/(#num)/(#any)/(#num)', function ($table, $id, $field1, $value1)
{
	$query = array
	(
		//sprintf('DELETE FROM "%s" WHERE "%s" = ?', $table, 'id'),
		//sprintf('UPDATE "%s" SET %s WHERE "%s" = ? and "%s" = "%s"', $table, implode(', ', $data), 'id', $field1, $value1),
		sprintf('DELETE FROM "%s" WHERE "%s" = ? and "%s" = "%s"', $table, 'id', $field1, $value1),
	);
...
ArrestDB::Serve('PUT', '/(#any)/(#num)/(#any)/(#num)', function ($table, $id, $field1, $value1)
{
	...
	else if (is_array($GLOBALS['_PUT']) === true)
	{
		...
		$query = array
		(
			//sprintf('UPDATE "%s" SET %s WHERE "%s" = ? and "%s" = ?', $table, implode(', ', $data), 'id', 'group_id'),
			//sprintf('UPDATE "%s" SET %s WHERE "%s"= %s and "%s"= %s', $table, implode(', ', $data), 'id', $id, $field1, $value1),
			//sprintf('UPDATE "%s" SET %s WHERE "id"="1" and "group_id"="1"'	, $table, implode(', ', $data)),
			//sprintf('UPDATE "%s" SET %s WHERE "%s" = ? and "%s" = 1', $table, implode(', ', $data), 'id', 'group_id'),
			sprintf('UPDATE "%s" SET %s WHERE "%s" = ? and "%s" = "%s"', $table, implode(', ', $data), 'id', $field1, $value1),
		);
